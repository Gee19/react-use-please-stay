...
export const usePleaseStay = ({
  ...
  useEffect(() => {
    ...
    const cachedFavicons = Object.entries(localStorage).filter(
      (key) => key.indexOf('usePleaseStayFavicon-') != -1,
    );

    if (!!cachedFavicons) {
      externalFavicons = faviconURIs.filter(
        (f) => /^(http|https):\/\/[^ "]+$/.test(f) && convertToBase64(f)
      );

      externalFavicons.forEach((f, index) =>
        localStorage.setItem(`usePleaseStayFavicon-${index}`, f),
      );
    }
